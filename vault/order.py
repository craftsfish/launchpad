...
class Order(models.Model):
	...
	系统内部标识一个具体的物理出货记录时，transaction的desc属性格式为{invoice排序号}.出货.{平台编号}
	...
	[订单内部描述]
	出货 | {invoice排序号}.出货.{平台编号} | (正常订单出货流程，根据订单状态，反应真实的仓库情况，应发/已发)
	刷单.回收 | {invoice排序号}.刷单.回收.{平台编号} |(不需要真实发货的刷单，立刻通过此条目中和正常出货的效果)
	刷单.应退 | {invoice排序号}.刷单.应收.{平台编号} | (需要回收实物的刷单，通过此条目记录下应收明细)
	刷单.入库 | {invoice排序号}.刷单.入库.{平台编号} | (需回收实物的刷单，通过此条目记录收货详情)
	刷单.发货 | 刷单.发货 | (根据具体情况的不同，刷单需要发出的货物，比如洗衣粉，肥皂，盐等)
	刷单.结算 | 刷单.结算 | (刷单的费用，实际订单费用+佣金)
	退货 | 退货 | (正常退货，对应的invoice状态为完成，则记录为一条收货记录。状态为删除或者取消则记录为一条换仓记录。)
	换货.收货 | 换货.收货 | (订单换货)
	换货.发货 | 换货.发货 | (订单换货)
	结算 | 结算 | (订单费用结算)
	返现 | 返现 | (给客户返利)
	...
	def create_or_update_invoice_shipment(self, organization, invoice_id, platform_commodity_id, commodities, quantity, delivery):
		...
		if transaction: #update
			...
			for c in commodities:
				Transaction.add_raw(task, "{}.出货.{}".format(invoice_id, platform_commodity_id),
